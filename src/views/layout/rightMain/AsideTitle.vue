<template>
  <div class="titleNavBox" :style="{ width: 'calc(100%)' }">
    <div style="position: relative">
      ...
      <div class="titleNav" @click.right.prevent="mouseRight">
        <el-tag
          ...
          :class="{
            title: true,
            active: getactive === item.activeIndex ? true : false,
...
export default {
  ...
  data() {
    return {
      ...
      getNavs: this.$store.getters.Navs,
      getactive: this.$store.getters.active,
      getCollapseState: this.$store.getters.CollapseState,
      getCrumbs: this.$store.getters.Crumbs
    };
  },
  computed: {
    ...mapGetters(["AsideTitle", "active", "CollapseState", "Crumbs", "Navs"])
  },
  watch: {
    AsideTitle(val) {
  ...
  methods: {
    ...
    move(direction) {
      if (direction === "left") {
        let num = 0;
        // console.log(this.navWidth, this.pageWidth);
        num += (this.navWidth - this.pageWidth + 186) / 3;
        // console.log(num, "num");
